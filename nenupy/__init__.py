...
def logdebug(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        signature = ", ".join("{}={!r}".format(*item) for item in kwds.items())
        log.debug(f"Calling {func.__qualname__}({signature}).")
        t0 = Time.now()
        result = func(*args, **kwargs)
        t1 = Time.now()
        log.info(f"{func.__qualname__} completed in {(t1 - t0).sec:.3f} sec.")
        return result
